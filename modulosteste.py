...
def mainrecordes():
    ...
    img = font.render('Digite seu nome:', True, (250, 250, 250))
...
class Game(InputBox):
    ...
    def frame2(self, ponto, vida):
        global event
        pg.font.init()
        pg.init()
        pg.display.set_caption("PySolvers Quiz Game")
        self.victory_line1 = 'Parabéns, você venceu.'
        ...
        self.pontos = ponto
        self.vidas = vida
        self.fim = pg.Rect(340, 490, 360, 60)
        ...
        while self.sair != True:
            ...
            self.pontuacao = self.texto_score.render(self.pontos, True, (200, 200, 0))
            self.vida = self.texto_score.render(self.vidas, True, (200, 200, 0))
            self.texto_fim = self.texto_score.render('Sair', True, self.cor_player)
            ...
            self.tela.blit(self.azul, (340, 490))
            ...
            elif self.rect.colliderect(self.fim):
                self.escolhe(self.rect, self.fim, 340, 490)
                if event.type == pg.MOUSEBUTTONDOWN:
                    self.escolhe(self.rect, self.fim, 340, 490)
                    break

            self.tela.blit(self.texto_fim, (470, 510))
    ...
    def frame3(self, ponto):
        ...
        while self.sair != True:
            ...
            # colisões
            if self.rect.colliderect(self.retry): #tente novamente
                self.escolhe(self.rect, self.retry, 340, 260)
                if event.type == pg.MOUSEBUTTONDOWN:
                    sleep(.1)
                    self.frame1()
            elif self.rect.colliderect(self.resultados):    #records
                self.escolhe(self.rect, self.resultados, 340, 360)
                if event.type == pg.MOUSEBUTTONDOWN:
    ...
    def recordes(self):
        ...
        # self.start = pg.Rect(340, 240, 360, 60)
        self.recordes = pg.Rect(340, 320, 360, 60)
        self.voltar = pg.Rect(340, 490, 360, 60)

        while self.sair != True:
            self.setting()

            # Textos
            self.texto_recordes = self.texto_final.render('Listar Recordes:', True, self.cor_player)
            self.texto_voltar = self.texto_score.render('Voltar', True, self.cor_player)

            # Background
            self.tela.blit(self.fundo, (0, 0))
            self.tela.blit(self.balloon, (0, 490))
            #self.tela.blit(self.prod, (250, 30))
            self.tela.blit(self.logo, (30, 90))
            self.tela.blit(self.azul, (340, 490))
            ...
            # colisões
            if self.rect.colliderect(self.voltar):
                self.escolhe(self.rect, self.voltar, 340, 490)
                if event.type == pg.MOUSEBUTTONDOWN:
                    sleep(.2)
                    self.menu()

            self.tela.blit(self.texto_recordes, (380, 50))
            self.tela.blit(self.texto_voltar, (480, 510))
